...
public class JSLBotBridge extends SLModule {
	...
	public String getDescription() {
		return "JSLBot bridge module for core SL services";
	}
			...
			for (final Parameter param: method.getParameters()) {
				if (firstParam) {
					...
					if (!warned&&!param.isAnnotationPresent(JSLBot.Param.class)) {
						SL.log("JSLBotBridge")
						  .severe("Malformed JSLBot CmdHelp/Param annotations in "+
	...
	public final String commitId() {
		return JSLBotBridgeBuildInfo.COMMITID;
	}
	
	public final Date getBuildDate() {
		return JSLBotBridgeBuildInfo.BUILDDATE;
	}
	...
	public static BotConfig getBotConfig() {
		...
		botconfig.put("handlers",
		              "Avatars,CnC,Sink,Health,Regions,Teleportation,Agent,Objects,Groups"
		              //todo ,net.coagulate.SL.Bots.BotInterface,net.coagulate.SL.Services.RegionMonitor"+".SimStats"
		);
		...
		if (!Config.getJSLBotBridgeSeat().isBlank()) {
			botconfig.put("homeseat",Config.getJSLBotBridgeSeat());
		}
		return botconfig;
	}
	...
	private class RecalcThread extends Thread {
		...
		public RecalcThread(final EventQueue event) {
			...
			log.info("Exiting - processed "+processed+" in "+UnixTime.duration(UnixTime.getUnixTime()-startTime,true)+
			         " with "+renames+" renames, "+caseCorrections+" case corrections, "+unknowns+" failed lookups,"+
			         oddLength+" odd lengths and "+noop+" NOOPs.");
			event.complete();
		}
